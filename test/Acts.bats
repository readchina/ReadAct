...
@test "works in Act.act_object" {
  result=$(mlr --csv join -j act_object -r work_id --np --ul -f data/Act.csv then cut -f act_object then uniq -a data/Work.csv | grep -ow -c "W[0-9]*" || true)
  [ "$result" -eq 0 ]
}

@test "individuals in Act.act_target" {
  result=$(mlr --csv join -j act_target -r person_id --np --ul -f data/Act.csv then cut -f act_object then uniq -a data/Person.csv | grep -ow -c "P[0-9]+" || true)
  [ "$result" -eq 0 ]
}
